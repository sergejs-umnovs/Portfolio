...
int main() {
	...
	unsigned int vertexShader = glCreateShader(GL_VERTEX_SHADER); // ñîçäà¸ì ïóñòûøêó
	...
	unsigned int redFragmentShader = glCreateShader(GL_FRAGMENT_SHADER); // ñîçäà¸ì ïóñòûøêó
	...
	unsigned int yellowFragmentShader = glCreateShader(GL_FRAGMENT_SHADER); // ñîçäà¸ì ïóñòûøêó
	...
	unsigned int VBO1, VAO1, VBO2, VAO2; // äëÿ õðàíåíèÿ id

	glGenVertexArrays(1, &VAO1);
	glGenBuffers(1, &VBO1); // äåëàé 1 áóôåð

	glGenVertexArrays(1, &VAO2);
	glGenBuffers(1, &VBO2);

	glBindVertexArray(VAO1);// ñäåëàé èç âûäåëåííîãî ïðîñòðàíñòâà ðàáî÷èé áóôåp
	glBindBuffer(GL_ARRAY_BUFFER, VBO1); // ñäåëàé èç âûäåëåííîãî ïðîñòðàíñòâà ðàáî÷èé áóôåð
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices1), vertices1, GL_STATIC_DRAW); // äîáàâü â áóôåð äàííûå
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);

	glBindVertexArray(VAO2);
	glBindBuffer(GL_ARRAY_BUFFER, VBO2);
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices2), vertices2, GL_STATIC_DRAW); // äîáàâü â áóôåð äàííûå
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0);
	glEnableVertexAttribArray(0);
